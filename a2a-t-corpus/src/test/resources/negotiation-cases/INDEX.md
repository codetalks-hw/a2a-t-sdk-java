...
| Id | API | Languages | Priority | Expectation | Summary |
|---|---|---|---|---|---|
| VAL-PROG-01 | `validateProposePromptAndDataFilling` | zh-CN | P0 | (NullPointerException) | 校验要数提议消息时未传入消息文本，直接抛出空指针异常且不调用大模型 |
| VAL-PROG-02 | `validateProposePromptAndDataFilling` | zh-CN | P1 | (IllegalArgumentException) | 校验要数提议消息时消息文本为空白，抛出参数异常（文档声明未验证，doc-gap 探针） |
| VAL-PROG-03 | `validateAcceptPromptAndDataFilling` | zh-CN | P0 | (NullPointerException) | 校验补数接受消息时未传入消息文本，直接抛出空指针异常且不调用大模型 |
| VAL-PROG-04 | `validateAcceptPromptAndDataFilling` | zh-CN | P1 | (IllegalArgumentException) | 校验补数接受消息时消息文本为空白，抛出参数异常（文档声明未验证，doc-gap 探针） |
| VAL-PROG-05 | `validateRejectPromptAndDataFilling` | zh-CN | P0 | (NullPointerException) | 校验修复目标拒绝消息时未传入消息文本，直接抛出空指针异常且不调用大模型 |
